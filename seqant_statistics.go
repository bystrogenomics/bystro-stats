...
func main() {
	...
	for {
		...
		if record[*heterozygotesColumnIdx] != *emptyFieldString {
			samples = append(samples, strings.Split(record[*heterozygotesColumnIdx], *primaryDelimiter)...)
		}

		if record[*homozygotesColumnIdx] != *emptyFieldString {
			samples = append(samples, strings.Split(record[*homozygotesColumnIdx], *primaryDelimiter)...)
		}
		...
		// Make siteTypes unique; struct{} takes up no additional space
		// http://stackoverflow.com/questions/9251234/go-append-if-unique
		seen := make(map[string]struct{})

		for _, val := range strings.Split(record[*siteTypeColumnIdx], *secondaryDelimiter) {
			for _, innerVal := range strings.Split(val, *primaryDelimiter) {
				if innerVal != *emptyFieldString {
					if _, found := seen[innerVal]; !found {
						seen[innerVal] = struct{}{}
						siteTypes = append(siteTypes, innerVal)
					}
				}
			}
		}

		for _, val := range strings.Split(record[*exonicAlleleFunctionColumnIdx], *secondaryDelimiter) {
			for _, innerVal := range strings.Split(val, *primaryDelimiter) {
				if innerVal != *emptyFieldString {
					if _, found := seen[innerVal]; !found {
						seen[innerVal] = struct{}{}
		...
		if hasDbSnpColumn && record[*dbSNPnameColumnIdx] != *emptyFieldString {
			for _, val := range strings.Split(record[*dbSNPnameColumnIdx], *secondaryDelimiter) {
				for _, innerVal := range strings.Split(val, *primaryDelimiter) {
					if innerVal != *emptyFieldString {
						hasDbSnp = true
					}
